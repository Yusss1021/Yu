...
```powershell
python main.py rules update --url https://example.com/my_rules.json --mode merge
```

带资产画像扫描：

```powershell
python main.py scan --target 127.0.0.1/32 --methods icmp --ports 2222,6379 --asset-profile docs/asset_profile.example.json
```

服务识别与漏洞匹配结果对比（CLI）：

```powershell
python main.py compare --base 16 --new 17
```

Web 前端（含并发任务和结果对比页面）：

```powershell
python main.py web --host 127.0.0.1 --port 5000 --max-concurrent 3
```
