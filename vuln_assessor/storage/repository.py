...
class ScanRepository:
    ...
    def initialize(self) -> None:
        with self._connect() as conn:
            journal_mode = "WAL" if os.name == "posix" else "DELETE"
            _ = conn.execute(f"PRAGMA journal_mode={journal_mode}")
            _ = conn.execute(
                """
            ...
            self._ensure_column(conn, "scans", "scan_type", "TEXT NOT NULL DEFAULT 'analysis'")
        ...
        with self._connect() as conn:
            ...
            _ = cursor.execute(
                ...
                INSERT INTO scans (
                    ...
                    total_hosts, total_services, total_risks, high_count, medium_count, low_count, report_path, scan_type
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
    ...
    def list_scans(self, limit: int = 20, scan_type: str | None = None) -> list[dict[str, Any]]:
        with self._connect() as conn:
            ...
            query = """
                SELECT id, target, started_at, total_hosts, total_services, total_risks,
                       high_count, medium_count, low_count, report_path, scan_type
                FROM scans
            """
            params: tuple[object, ...]
            if scan_type:
        ...
        with self._connect() as conn:
            ...
            row = conn.execute(
                ...
                SELECT id, target, methods, ports, started_at, finished_at, duration_seconds,
                       total_hosts, total_services, total_risks, high_count, medium_count, low_count,
                       report_path, scan_type
                FROM scans
                WHERE id = ?
                """,
                (scan_id,),
    ...
    def delete_scan(self, scan_id: int) -> dict[str, Any] | None:
        ...
        with self._connect() as conn:
            _ = conn.execute("DELETE FROM vulnerabilities WHERE scan_id = ?", (scan_id,))
            _ = conn.execute("DELETE FROM services WHERE scan_id = ?", (scan_id,))
            _ = conn.execute("DELETE FROM assets WHERE scan_id = ?", (scan_id,))
            _ = conn.execute("DELETE FROM scans WHERE id = ?", (scan_id,))
        return scan
