...
class HtmlReportGenerator:
    ...
    def generate(
        ...
        normalized_report_type = "asset_discovery" if report_type == "asset_discovery" else "analysis"
        is_asset_report = normalized_report_type == "asset_discovery"
        report_title = "资产发现报告" if is_asset_report else "漏洞匹配与服务识别报告"

        report_dir = output_dir / f"{normalized_report_type}_{report_name}"
        ...
        asset_port_records = sum(len(asset.open_ports or []) for asset in assets or [])
        ...
        html = template.render(
            ...
            is_asset_report=is_asset_report,
            ...
            low_count=low_count,
            asset_port_records=asset_port_records,
            subnet_group_count=len(subnet_groups),
            assets=assets,
            services=services,
            top_risks=top_risks,
