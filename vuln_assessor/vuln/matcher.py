...
class VulnerabilityMatcher:
    ...
    def __init__(self, rule_file_path: Path) -> None:
        ...
        self._service_index: dict[str, list[dict[str, object]]] = {}
        self._product_index: dict[str, list[dict[str, object]]] = {}
        self._service_needles: tuple[str, ...] = ()
        self._product_needles: tuple[str, ...] = ()
        self._build_indexes()
    ...
    def match(self, services: list[ServiceFingerprint]) -> list[VulnerabilityFinding]:
        ...
        for service in services:
            for rule in self._candidate_rules(service):
    ...
    def _build_indexes(self) -> None:
        service_index: dict[str, list[dict[str, object]]] = {}
        product_index: dict[str, list[dict[str, object]]] = {}

        for rule in self.rules:
            rule_service = self._rule_service(rule)
            rule_product = self._rule_product(rule)
            if rule_service:
                service_index.setdefault(rule_service, []).append(rule)
            if rule_product:
                product_index.setdefault(rule_product, []).append(rule)

        self._service_index = service_index
        self._product_index = product_index
        self._service_needles = tuple(sorted(service_index, key=len, reverse=True))
        self._product_needles = tuple(sorted(product_index, key=len, reverse=True))

    def _candidate_rules(self, service: ServiceFingerprint) -> list[dict[str, object]]:
        ...
        if detected_service:
            for needle in self._service_needles:
                if needle not in detected_service:
                    continue
                self._append_candidates(
                    candidates,
                    seen_rule_ids,
                    self._service_index.get(needle, []),
                    service.port,
                )

        if detected_product:
            for needle in self._product_needles:
                if needle not in detected_product:
                    continue
                self._append_candidates(
                    candidates,
                    seen_rule_ids,
                    self._product_index.get(needle, []),
                    service.port,
                )
    ...
    def _append_candidates(
        self,
        candidates: list[dict[str, object]],
        seen_rule_ids: set[int],
        rules: list[dict[str, object]],
        port: int,
    ) -> None:
        for rule in rules:
            if not self._rule_port_matches(rule, port):
                continue
            rule_id = id(rule)
    ...
    def _rule_port_matches(self, rule: dict[str, object], port: int) -> bool:
    ...
    def _rule_service(self, rule: dict[str, object]) -> str:
        return str(rule.get("service", "")).strip().lower()

    def _rule_product(self, rule: dict[str, object]) -> str:
        return str(rule.get("product", "")).strip().lower()
