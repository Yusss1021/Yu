...
class ServiceFingerprintEngine:
    ...
    def fingerprint(
        ...
        if self.nmap_installed:
            return self._fingerprint_with_nmap(assets, fallback_ports, nmap_arguments=nmap_arguments)
        if require_nmap:
            raise NmapUnavailableError("nmap 不可用，无法执行漏洞匹配与服务识别。请先在当前系统安装 nmap。")
    ...
    def _fingerprint_with_nmap(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(
                    self._scan_host_with_nmap,
                    asset.ip,
                    asset.open_ports or fallback_ports,
                    nmap_arguments,
                ): asset.ip
                for asset in assets
            }
    ...
    def _scan_host_with_nmap(
        ...
        base_arguments = list(nmap_arguments or ["-sV", "-Pn", "--version-intensity", "5"])
        command = [
            "nmap",
            *base_arguments,
            "-p",
